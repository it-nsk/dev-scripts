...
DEV_TOOLS_PROJECT_DIR="$TMP/project" "$ROOT/bin/dev-tools-global" version | grep -qx '0.2.0'
...
DEV_TOOLS_INSTALL_DIR="$TMP/bin" \
    "$ROOT/install.sh"
"$TMP/bin/dev-tools" version | grep -qx '0.2.0'
...
sed "s/DEV_TOOLS_VERSION='0.2.0'/DEV_TOOLS_VERSION='0.1.0'/" \
    "$ROOT/bin/dev-tools-global" >"$TMP/bin/dev-tools-old"
chmod +x "$TMP/bin/dev-tools-old"
DEV_TOOLS_UPDATE_URL="file://$ROOT/bin/dev-tools-global" \
DEV_TOOLS_INSTALL_PATH="$TMP/bin/dev-tools-old" \
    "$TMP/bin/dev-tools-old" self-update | grep -q 'Updated dev-tools: 0.1.0 -> 0.2.0'
"$TMP/bin/dev-tools-old" version | grep -qx '0.2.0'
